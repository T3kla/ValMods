...
namespace Areas
{
    ...
    public static class FloatArrayExtensions
    {
        ...
        public static Vector3 ToVector3(this float[] ar)
        {
            Vector3 v = Vector2.zero;

            try
            {
                return new Vector3(ar[0], ar[1], ar[2]);
            }
            catch (Exception e)
            {
                Main.Log.LogError($"Failed Conversion 'float[]' to 'Vector3'\n{e.Message}\n{e.StackTrace}");
                return v;
            }
        }
    }
